...
impl Tool for WriteFileTool {
    ...
    async fn execute(&self, args: &serde_json::Value) -> Result<String> {
        ...
        if args.path.trim().is_empty() {
            return Err(eyre!("write_file: 'path' no puede estar vacio"));
        }
                ...
                tokio::fs::create_dir_all(parent)
                    ...
                    .map_err(|error| match error.kind() {
                        std::io::ErrorKind::PermissionDenied => ToolError::PermissionDenied {
                            message: format!(
                                "write_file: permiso denegado creando '{}'",
                                parent.display()
                            ),
                        ...
                        _ => ToolError::Generic {
                            message: format!(
                                "write_file: no se pudo preparar '{}': {}",
                                parent.display(),
                                error
                            ),
        ...
        tokio::fs::write(&args.path, args.content)
            ...
            .map_err(|error| match error.kind() {
                ...
                _ => ToolError::Generic {
                    ...
                    hint: Some(
                        "Verifica que la ruta sea valida y que el disco este disponible"
                            .to_string(),
                    ),
                },
            })?;
